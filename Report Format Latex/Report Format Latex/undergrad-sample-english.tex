...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% First line of title
\def\disstitleone{Adaptive Artificial Intelligence Agent in Game Development}   
...
%\subsection{Multi-Agent Support and Role Specialization}

%Although this project focuses primarily on the player agent, the system architecture and PPO trainer support multi-agent training. Future work will introduce adversarial agents—e.g., enemies that learn to challenge the player agent. These agents will have:

%\begin{itemize}
%\item Independent reward functions and behavior names
%\item Separate PPO configurations and trainers
%\item Inter-agent dynamics (chasing, evading, interfering)
%\end{itemize}

%This adversarial setup encourages co-evolution and leads to emergent gameplay behaviors, making it especially valuable for automated playtesting and enemy AI development

%\subsection{Hyperparameter Adjustment and Curriculum Design (Future Scope)}

%Although this project uses a single static environment, the system is designed to support Curriculum Learning in future work. By incrementally increasing difficulty (e.g., platform complexity, enemy count), agents can learn progressively and improve robustness.

%Reward values are also exposed via the RewardConfigSO ScriptableObject, allowing live tweaking between training sessions without changing core code.
...
Over the course of development, approximately 30 independent training runs were conducted. Each run was configured with the PPO algorithm and executed between 1 million to 2 million training steps. After each run, results were analyzed using cumulative reward trends, performance metrics, and in-game behavior. This iterative process enabled fine-tuning of:

\begin{itemize}
\item Reward values and shaping penalties (via \texttt{RewardConfigSO})
...
These adjustments were made to improve sample efficiency, stabilize learning, and eliminate unintended exploitative behaviors (e.g., repeated jumping or idling for reward).

\subsection{Environment Configuration}

All experiments were conducted within a single handcrafted 2D platformer level that includes:

\begin{itemize}
\item Static and moving platforms
\item Collectible items (coins, checkpoints)
\item Hazardous elements (spikes, pits)
\item Melee enemies controlled by state machines
\end{itemize}

To prevent memorization of fixed obstacle patterns, a dynamic episode reset system was used. After each death or episode termination, the player’s spawn point was randomized from a predefined list. Simultaneously, all environment elements—coins, checkpoints, enemies were reset to their initial state. This strategy ensured that the agent learned generalizable skills rather than memorizing deterministic paths.

\subsection{Evaluation Protocol}

Agent performance was periodically evaluated during and after training in inference mode. In this phase:

\begin{itemize}
\item The environment was reset using the same randomization logic.
\item No exploration noise or entropy was applied; the trained policy operated deterministically.
\item Metrics such as episode reward, steps taken, coin collection rate, and death count were tracked using \texttt{RewardLogger.cs} and TensorBoard.
\end{itemize}

By comparing evaluation performance across runs, the experiments revealed the impact of reward modifications, observation tweaks, and PPO settings on learned behavior quality.

\subsection{Focus on Player Agent}

Due to time constraints, the current experiments focused exclusively on training the player agent. Enemy agents operated with scripted state machines and were used to test obstacle avoidance, navigation under pressure, and combat timing. Future iterations aim to train enemy agents using a similar PPO-based setup, treating the trained player agent as a fixed adversary.
